<?php
  ...
  $member_no = $sql -> fetch_array();
    ...
    $sql = mq("insert into review(id, title, description, file, memberNum, genre, kategorie, review_date, genre_title, country)
                values('".$_SESSION['id']."', '".$title."', '".$description."', '".$o_name."', '".$member_no['mem_no']."', '".$genre."', '".$kategorie."',now(),'".$genre_title."', '".$country."')");
    echo "<script>alert('글쓰기 성공했습니다.'); location.href='http://localhost/WebTeamProject/curture_page/page/Japan_review_page.php';</script>";
    } else {
      $sql = mq("insert into review(id, title, description, memberNum, genre, kategorie, review_date, , genre_title, country)
                  values('".$_SESSION['id']."', '".$title."', '".$description."', '".$member_no['mem_no']."', '".$genre."', '".$kategorie."',now(), '".$genre_title."', '".$country."')");
      echo "<script>alert('글쓰기 성공했습니다.'); location.href='http://localhost/WebTeamProject/curture_page/page/Japan_review_page.php';</script>";
  }
