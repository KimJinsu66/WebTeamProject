      ...
      <tr height="150px">
        ...
        <td>
          <div class="table_content">
            ...
            <div><small>이름이나 생년월일, 성별 등의 정보가 변경되었다면 본인확인을 통해 정보를 수정할 수 있습니다.</small></div>
            <div>수정</div>
          </div>
        </td>
      </tr>
...
<?php }
  ...
  <div class="">
    ...
    $sql = mq("select * from members_bookmark as book, members as mem where book.membersNum = mem.mem_no and id = '".$_SESSION['id']."'");
    $result = $sql -> fetch_array();
    ...
    for($i=0; $i<count($bookmark_array); $i++){  
      ...
      $result2 = $sql2 -> fetch_array(); ?>
        <tr>
          <td>
            <!-- 즐겨찾기 번호에 해당하는 title을 보여주고, 링크로 해당하는 url review_no로 보내주면 해당 게시글로 바로 이동한다 -->
            <a href="./japan_review_read.php?review_no=<?=$bookmark_array[$i]?>"><?=$result2['title']?></a>      
          </td> 
        </tr>
    <?php }?>
      </table>  
   ...
   <div class="">
    ...
    for($i=0; $i<$low; $i++){  
      $result = $sql -> fetch_array(); ?>      
        <tr>
          <td>
            <!-- for문을 통해 순차대로 db를 가져온는데 해당하는 title 을 누르면 링크로 해당하는 url review_no 로 보내줘서 바로 게시글로 이동-->
            <a href="./japan_review_read.php?review_no=<?=$result['review_no']?>"><?=$result['title']?></a>      
          </td> 
        </tr>
    <?php }?>
      </table>  
